...
export function MigratePosts() {
  ...
  const migrateAllPosts = async () => {
    if (!confirm('This will migrate all 4 existing posts to Supabase. Continue?')) {
      return;
    }
    ...
    for (let i = 0; i < postsToMigrate.length; i++) {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.error(`Failed to migrate post "${post.title}":`, data.error);
        }
      } catch (error) {
    ...
    if (successCount === postsToMigrate.length) {
      toast.success(`Successfully migrated all ${successCount} posts to Supabase!`);
    } else {
      toast.warning(`Migrated ${successCount} out of ${postsToMigrate.length} posts. Check console for errors.`);
    }
  };

  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
      <h2 className="text-xl mb-4">Migrate Existing Posts to Supabase</h2>
      <p className="text-sm text-gray-700 mb-4">
        Click the button below to migrate your 4 existing blog posts to the Supabase database. 
      ...
      {migrating ? (
        <div className="space-y-2">
          ...
          <p className="text-sm text-gray-600">Migrating posts... {Math.round(progress)}%</p>
        </div>
      ) : (
        <button
